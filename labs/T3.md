...
<http://tahiti.prism.uvsq.fr/jtiger/static/support/livrable-T3.zip>.  Pour
...
~~~
    ...
    arguments::= [ argument { `,` argument } ] (AST de type LinkedList<Exp>, liste possiblement vide d'arguments)
    argument::= exp                            (AST de type Exp, un argument est toute expression valide)
~~~
...
~~~
    ...
    typesDec ::= { typeDec }        (AST de type TypesDec, un ensemble de déclarations de type)
...
~~~
    ...
    fields ::=                    (AST de type LinkedList<Field>, list vide ou pas de Field)
        [ id `:` id  {`,` id `:` id } ]
    ...
    efields ::= [ id `=` exp { `,` id `=` exp } ] (AST de type LinkedList<ExpField>, liste d'affectations)
~~~

Arrivés là, vous reconnaissez tout Tiger ! Félicitations :)

<img src='http://tahiti.prism.uvsq.fr/jtiger/static/support/Tiger.jpg' height='100px'>
